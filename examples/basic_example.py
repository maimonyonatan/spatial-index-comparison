...
try:
    ...
    print("❌ Import error: Make sure dependencies are installed with 'uv sync'")
...
def basic_example():
    ...
    try:
        ...
        for name, stat in stats.items():
            print(f"  {name}:")
            print(f"    Build time: {stat.get('build_time', 0):.4f} seconds")
            print(f"    Memory usage: {stat.get('memory_usage_mb', 0):.2f} MB")
            if 'r2_score' in stat:
                print(f"    R² Score: {stat['r2_score']:.4f}")
        
        # Step 5: Execute sample queries
        print("\n🔍 Executing sample queries...")
        
        # Point query
        query_lat, query_lon = float(coordinates[100][0]), float(coordinates[100][1])
        ...
        range_results = query_engine.range_query(
            float(lat_center - lat_range/2), float(lat_center + lat_range/2),
            float(lon_center - lon_range/2), float(lon_center + lon_range/2)
        )
        ...
        print("  2. Use the Python API in your own scripts")
        print("  3. Try with your own dataset using the GUI")
